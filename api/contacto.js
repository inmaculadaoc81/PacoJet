...
module.exports=async(req,res)=>{
  ...
  try{
    ...
    await transporter.sendMail({
      from:`"PacTech" <${process.env.SMTP_USER}>`,
      to:process.env.CONTACT_EMAIL||process.env.SMTP_USER,
      replyTo:email,
      subject:"Nueva consulta PacTech - pacojetech.com.es",
      text:`Nombre: ${nombre}\nTeléfono: ${telefono}\nEmail: ${email}\nEquipo: ${equipo}\n\nConsulta:\n${mensaje}`
    });

    return res.status(200).json({ok:true});
  }catch(error){
    console.error("PacTech SMTP error",{
      message:error?.message,code:error?.code,response:error?.response,command:error?.command
    });
